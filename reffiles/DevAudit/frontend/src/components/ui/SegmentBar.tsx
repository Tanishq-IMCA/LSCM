...
function useCountUp(target: number, delay: number, duration = 1.2, enabled = true) {
...
export function SegmentBar({
  ...
  const count = useCountUp(value, delay + 0.05, 1.2, inView);
    ...
    <div ref={ref} className="flex items-center gap-3 w-full">
      ...
      <div className="relative flex gap-[2px] flex-1 items-center">
        {Array.from({ length: segments }).map((_, i) => {
          ...
          const targetOpacity = isFilled ? 0.18 + progress * 0.82 : 1;
          return (
            <motion.div
              key={i}
              initial={{ scaleX: 0, opacity: 0 }}
              animate={inView ? { scaleX: 1, opacity: targetOpacity } : {}}
              transition={{
                delay: delay + i * 0.016,
                duration: 0.22,
