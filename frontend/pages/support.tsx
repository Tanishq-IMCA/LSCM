        ...
        <div className="mt-10 grid min-h-[560px] gap-5 lg:grid-cols-[0.35fr_0.65fr]">
          ...
          <section className="flex flex-col border border-white/[0.08] bg-white/[0.035]">
            {!ticket ? <div className="flex flex-1 items-center justify-center p-10 text-center text-sm text-white/35">Select a ticket or open a new one.</div> : <>
              <header className="border-b border-white/[0.08] p-5"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] uppercase tracking-[0.25em] text-[var(--accent)]">{ticket.queryType} · {ticket.queryTopic}</p><h2 className="mt-2 text-xl uppercase tracking-[0.08em] text-white">{ticket.orderNumber ? `Order ${ticket.orderNumber.slice(0, 8)}` : 'General support'}</h2></div><div className="flex items-center gap-3"><PinnedMessagesButton messages={messages} onTogglePin={togglePin} /><span className="text-[10px] uppercase text-white/35">{ticket.status}</span>{ticket.status === 'open' && <button type="button" onClick={() => void transition('close')} className="border border-red-300/30 px-3 py-2 text-[9px] uppercase tracking-[0.15em] text-red-200/70">Close</button>}</div></div></header>
