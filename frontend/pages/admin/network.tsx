...
export default function NetworkManagerPage() {
  ...
  const openModule = (name: string) => setMessage(`${name} module is ready for configuration.`);
        ...
        <div className="mt-8 grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
          ...
          <section className="glass-panel min-h-[260px] border-dashed border-white/15 p-7 md:p-9">
            ...
            <div className="mt-6 grid gap-3">
              {['Announcements', 'Moderator', 'Events'].map(module => (
                <button key={module} type="button" onClick={() => openModule(module)} className="border border-white/10 bg-white/[0.025] px-4 py-4 text-left text-xs uppercase tracking-[0.16em] text-white/65 transition hover:border-[var(--accent)]/50 hover:bg-[var(--accent)]/[0.06] hover:text-white">
                  {module}
                </button>
              ))}
            </div>
