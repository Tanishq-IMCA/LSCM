...
function MiniGallery({ images, name }: { images: string[]; name: string }) {
  ...
  return <div className="relative h-44 overflow-hidden border border-white/[0.08] bg-white/[0.025]"><img src={items[active % items.length]} alt={name} className="h-full w-full object-cover opacity-80 transition duration-500" /><div className="absolute bottom-2 left-3 right-3 flex h-px gap-0.5">{items.map((_, index) => <span key={index} className={`h-px flex-1 ${index === active % items.length ? 'bg-[var(--accent)]' : 'bg-white/20'}`} />)}</div></div>;
}