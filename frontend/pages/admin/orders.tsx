...
export default function AdminOrdersPage() {
  ...
  const groupedByStatus = useMemo(() => (Object.keys(STATUS_LABELS) as Status[]).map(status => {
    const groups = new Map<string, AdminOrder[]>();
    visible.filter(order => order.status === status).forEach(order => {
      const key = `${order.display_name} · ${order.email}`;
      groups.set(key, [...(groups.get(key) || []), order]);
    });
    return { status, groups: Array.from(groups.entries()) };
  }).filter(section => section.groups.length > 0), [visible]);

  const statusContainerClass: Record<Status, string> = {
    awaiting_approval: 'border-yellow-300/25 bg-yellow-200/[0.02] shadow-[0_0_30px_rgba(250,204,21,0.08)]',
    approved: 'border-purple-400/30 bg-purple-400/[0.025] shadow-[0_0_30px_rgba(168,85,247,0.12)]',
    finished: 'border-white/[0.1] bg-white/[0.015] grayscale opacity-70',
  };
  ...
  return (
    <main className="min-h-screen pt-24"><Header /><section className="mx-auto max-w-[1700px] px-6 py-16 md:px-10 xl:px-14">
      ...
      <div className="mt-8 flex flex-col gap-3 md:flex-row"><form className="flex flex-1 gap-2" onSubmit={event => { event.preventDefault(); void loadOrders(search); }}><input value={search} onChange={event => setSearch(event.target.value)} className="input-glass min-w-0 flex-1 px-4 py-3 text-sm text-white" placeholder="Search order number, name or email" /><button className="bg-[var(--accent)] px-5 py-3 text-[10px] uppercase tracking-[0.2em] text-black">Search</button></form><select value={activeStatus} onChange={event => setActiveStatus(event.target.value as 'all' | Status)} className="input-glass px-4 py-3 text-xs uppercase tracking-[0.16em] text-white"><option value="all" className="bg-[#100b1d]">All orders</option>{(Object.keys(STATUS_LABELS) as Status[]).map(status => <option key={status} value={status} className="bg-[#100b1d]">{STATUS_LABELS[status]}</option>)}</select></div>
       {loading ? <p className="py-16 text-center text-xs uppercase tracking-[0.2em] text-white/35">Loading orders...</p> : groupedByStatus.length === 0 ? <section className="mt-8 glass-panel p-12 text-center text-sm uppercase tracking-[0.2em] text-white/40">No orders found.</section> : groupedByStatus.map(({ status, groups }) => <section key={status} className={`mt-8 border p-6 md:p-8 ${statusContainerClass[status]}`}><div className="flex items-center justify-between border-b border-white/[0.08] pb-5"><div><p className="text-[10px] uppercase tracking-[0.25em] text-white/35">Order state</p><h2 className="mt-2 text-xl uppercase tracking-[0.08em] text-white">{STATUS_LABELS[status]}</h2></div><span className="text-[10px] uppercase text-white/30">{groups.reduce((count, [, items]) => count + items.length, 0)} items</span></div>{groups.map(([customer, customerOrders]) => <div key={customer} className="mt-5 border border-white/[0.08] bg-black/10 p-4"><div className="flex items-center justify-between border-b border-white/[0.08] pb-4"><h3 className="text-sm uppercase tracking-[0.08em] text-white">{customer}</h3><span className="text-[10px] uppercase text-white/30">{customerOrders.length} items</span></div><div className="mt-4 space-y-3">{customerOrders.map(order => <article key={order.id} className="border border-white/[0.08] bg-white/[0.035] p-4"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><div className="flex flex-wrap items-center gap-3"><h4 className="text-sm uppercase tracking-[0.08em] text-white">{order.product_name || order.productName}</h4><span className="border border-white/10 px-2 py-1 text-[9px] uppercase text-[var(--accent)]">{STATUS_LABELS[order.status as Status] || order.status}</span></div><p className="mt-2 text-[10px] uppercase tracking-[0.14em] text-white/30">Order {order.order_number} · Qty {order.quantity} · {order.category}</p></div><select value={order.status} onChange={event => void transition(order, event.target.value as Status)} className="input-glass px-3 py-2 text-[10px] uppercase text-white">{(Object.keys(STATUS_LABELS) as Status[]).map(option => <option key={option} value={option} className="bg-[#100b1d]">{STATUS_LABELS[option]}</option>)}</select></div></article>)}</div></div>)}</section>)}
    </section><Footer /></main>
  );
}