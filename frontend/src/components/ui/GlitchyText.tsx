...
export default function GlitchyText({
  ...
  // Split last word for accent color
  const lastSpace = displayText.lastIndexOf(' ');
  const baseText = lastSpace >= 0 ? displayText.slice(0, lastSpace + 1) : '';
  const tailText = lastSpace >= 0 ? displayText.slice(lastSpace + 1) : displayText;
  ...
  return (
    ...
    >
      {baseText}
      <span style={{ color: 'var(--accent)' }}>{tailText}</span>
    </Tag>
  );
}
